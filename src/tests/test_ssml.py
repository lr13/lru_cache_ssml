...
class TestParseSSML(unittest.TestCase):
    ...
    def test_throw_on_invalid_tag_attributes(self):
        self.assertRaises(Exception, lambda: parseSSML("<speak foo='bar'></speak>")) #single quote is the issue here
        self.assertRaises(Exception, lambda: parseSSML("<speak foo></speak>")) # dictionnary value is not present
